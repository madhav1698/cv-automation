...
class AuditIntel:
    ...
    def render_intelligence(funnel_body, market_body, stats, colors):
        ...
        if funnel_body is not None:
            for w in funnel_body.winfo_children():
                w.destroy()

            steps = [
                ("APPLIED", counts["Applied"], colors["accent"]),
                ("IN PROCESS", counts["Interview"], "#3B82F6"),
            ...
            for label, val, color in steps:
                ...
                progress = (val / (max(counts.values()) or 1))
        ...
        for w in market_body.winfo_children():
            w.destroy()
        countries = {}
        for d in stats.values():
